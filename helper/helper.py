"""Helper functions shared across the TiDE feature selection library."""
...
def fitness(
    ...
    if test is None and cv is not None:
        ...
        splitter = check_cv(cv=cv, y=y_train, classifier=is_clf)
        fold_scores = []
        y_pred = cross_val_predict(pipeline, X_train, y_train, cv=splitter, n_jobs=1)
        for train_idx, test_idx in splitter.split(X_train, y_train):
            ...
            if is_clf:
                try:
                    y_hat = np.asarray(y_hat).astype(y_train.dtype, copy=False)
        ...
        if is_clf:
            try:
                y_pred_series = y_pred_series.astype(y_train.dtype)
            except (TypeError, ValueError):
                pass
        return score, y_train.reset_index(drop=True), y_pred_series, fold_scores, fold_std
